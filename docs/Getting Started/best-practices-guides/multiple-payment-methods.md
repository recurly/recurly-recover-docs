...
* **If you are using gateway tokens**, ensure you have the gateway token that represents each payment method for the consumer and that the proper gateway with access to that token is onboarded. Depending on the gateway, you may also need to provide the expiration date and NTID you keep on file for the customer's subscription. Exceptions are: Stripe, Braintree, PayPal Complete.
