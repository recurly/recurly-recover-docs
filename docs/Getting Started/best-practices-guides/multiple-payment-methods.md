...
* **If you are using gateway tokens**, take special care to ensure you are passing the correct `gateway_code` when submitting invoices. Since gateway tokens and the underlying payment method are usually tied to a specific account (with few exceptions), sending in the wrong gateway code could result in an error.
* **When using gateway tokens and multiple payment methods**, not only should you ensure that you are sending the right code (see previous bullet), but also sending more than one token for the same payment method if you wish to retry on all of your configured gateways. Ensure you include a gateway token for the same payment method for every gateway you want us to attempt against.
...
* **If you are using gateway tokens**, ensure you have the gateway token or tokens (when using multiple methods) that represents each payment method for the consumer and that the proper gateway with access to that token is added to your site. Depending on the gateway, you may also need to provide the expiration date and NTID you keep on file for the customer's subscription. Exceptions are: Stripe, Braintree, PayPal Complete.
